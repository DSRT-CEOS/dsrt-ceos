      ...
      <div ref={searchRef} className={`flex-1 max-w-xl relative ${showMobileSearch ? "fixed inset-x-0 top-0 z-50 bg-card p-2 max-w-full border-b border-border" : "hidden md:block"}`}>
        <div className="relative">
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)}
            ...
            placeholder="Search tenders, projects, bills..." className="pl-9 pr-9 h-8 text-xs bg-secondary border-border" />
          {searching && <Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-orange-400 animate-spin" />}
          {showMobileSearch && (
            <button onClick={() => { setShowMobileSearch(false); setSearch(""); }}
              className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground md:hidden">✕</button>
          )}
        </div>
      ...
      <div className="flex items-center gap-1 md:gap-2 ml-auto">
        ...
        <div ref={notifRef} className="relative">
          <Button variant="ghost" size="sm" className="h-8 w-8 p-0 relative" onClick={() => setShowNotifs(!showNotifs)}>
            ...
            {notifCount > 0 && (
              <span className="absolute top-0.5 right-0.5 bg-orange-500 text-white text-[9px] rounded-full w-3.5 h-3.5 flex items-center justify-center font-bold border border-card">
                ...
                {notifications.length === 0 ? (
                  ...
                  notifications.map((n: any, i: number) => (
                    <div key={i} className="p-3 border-b border-border/50 hover:bg-secondary/30">
                      <p className="text-orange-400 text-[10px] font-semibold uppercase tracking-wider">{n.type.replace(/_/g, " ")}</p>
                      <p className="text-foreground text-sm mt-1">{n.count} item{n.count > 1 ? "s" : ""}</p>
                      <div className="mt-2 space-y-1">
                        {n.items.slice(0, 3).map((item: any, idx: number) => (
                          <p key={idx} className="text-muted-foreground text-xs truncate">• {item.workName || item.name || item.billNumber || item.department}</p>
                        ))}
                      </div>
                    </div>
                  ))
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="sm" className="gap-2 h-8 px-2">
              <div className="w-6 h-6 bg-orange-500/15 border border-orange-500/30 rounded-full flex items-center justify-center">
