...
export async function GET() {
  try {
    ...
    const staff = await prisma.staffMember.findMany({ where: { companyId: u.companyId, isActive: true }, orderBy: { name: "asc" } });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const staff = await prisma.staffMember.create({
      data: {
        companyId: u.companyId, name: b.name, designation: b.designation,
        qualification: b.qualification || null,
        experienceYears: b.experienceYears ? parseInt(b.experienceYears) : null,
        aadhaarNumber: b.aadhaarNumber || null, panNumber: b.panNumber || null,
